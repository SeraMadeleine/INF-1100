...
int main(void)
{
    ...
    /* Create the suface in RAM that we manipulate the pixels of */ // lærret til å tegne på
    ...
    for (int i = 0; i < 1092; i++) // for-loops that run through all the triangels inside the teapot data  
    {
        draw_triangle(surface, &teapot_model[i]); 
        // &teapot_model[i] is an array and we wish to acses all the values in the array, thefor [i]. 
    ...
    // kjører i en evig loop, while !done = 1 
    done = 0;
    while (!done) {
